...
const checkLength = (errorMessage) => (value) =>
  value && value.length && value.length > 4 ? { success: true, value } : { success: false, message: errorMessage };
...
const signin = {
  name: "signin",
  sql:
    "INSERT INTO Users (id, username, password, role_id) VALUES ( ${id}, ${username}, ${password}, (SELECT id FROM Roles where name='Member') );",
  ...
  beforeQuery: (query, params, { encrypt }) =>
    Object.assign({}, params, { password: encrypt(params.password), id: uuidv4() }),
};
...
const login = {
  name: "login",
  sql:
    "SELECT Users.id as user_id, Users.username as username, Roles.name as role FROM Users, Roles where username=${username} and password=${password} and Users.role_id = Roles.id;",
  restricted: ["Public"], // Mind the Capital
  params: {
    username: checkLength("username should be longer"),
    password: checkLength("password should be longer"),
...
const islogged = {
  name: "islogged",
  sql:
    "SELECT Users.username as username, Roles.name as role FROM Users, Roles WHERE Users.role_id = Roles.id AND Users.id=${user_id};",
